...
export default function ChromeExtension() {
  return (
    <article className="project-document project-document--chrome">
...
function HeroSection() {
  return (
    <TitleSection title="사내 생산성 Chrome Extension" subtitle="업무 생산성 향상을 위한 크롬 확장 앱" description="React + Spring Cloud + Chrome API" />
  );
}
...
function OverviewSection() {
  return (
    <Box sx={{mb:{xs:2,sm:3}}}>
      ...
      <Typography variant="body1" component="p" sx={{mb:1.5}}>
        Keycloak SSO와 토큰 갱신을 연결해 반복 로그인을 줄이는 흐름을 구현한 기록입니다. 토큰 검증 외에 사용자별 데이터 권한, 갱신 실패와 재로그인 조건도 확인해야 합니다.
      </Typography>
    </Box>
  );
}
...
function FeaturesSection() {
  return (
    <Box sx={{mb:{xs:2,sm:3}}}>
      <Typography variant="h6" gutterBottom component="h2" className="project-section-heading">
        📸 실제 작동 화면
      </Typography>
      <Box sx={{mb:1}}>
        <video controls preload="none" autoPlay muted loop playsInline style={{ width: '100%'}}>
...
`;
  ...
  return (
    <Box sx={{mb:{xs:2,sm:3}}}>
      ...
      <Box sx={{mb:3,p:2}} className="project-note">
        {/* Frontend 섹션 */}
        <Typography variant="h6" gutterBottom fontWeight="600" component="h3" className="project-subsection-heading">
          • Frontend
        </Typography>
        <Box sx={{pl:3,mb:2}}>
        ...
        <Box sx={{pl:3,mb:2}}>
          ...
          <Typography component="div" sx={{mb:0.5,fontWeight:'medium'}}>
            <b>외부 통신 모듈(External Service): [Spring WebFlux (비동기, Reactive)]</b>
          </Typography>
          <Typography component="div" sx={{mb:2,fontWeight:'medium'}}>
        ...
        <Box sx={{pl:3,mb:1}}>
          ...
          <Box sx={{pl:4,mb:1}}>
            <Typography component="div" sx={{mb:0.3}}>
              공공데이터 포털(날씨 API)
            </Typography>
            <Typography component="div" sx={{mb:0.3}}>
              깃랩 MR 데이터 요청
      ...
      <ZoomableImageModal imageSrc="/images/chrome-keycloak.png" altText="Chrome Extension 시스템 아키텍처" sx={{mb:-1}} />

      <Typography component="li" sx={{mb:0.5}}>
        PKCE를 통해 토큰 발급 과정
      </Typography>
      <ZoomableImageModal imageSrc="/images/pkce.png" altText="PKCE를 통해 토큰 발급 과정" sx={{mb:-1}} />
      <Typography variant="body2" component="p" sx={{mt:1.5,mb:2}}>
        PKCE 그림과 아래 인증 함수 목록은 2025년 개발 기록입니다. 공개 프론트 체크아웃의 background.js에는 탭 열기만 남아 있어, 같은 소스에서 최종 인증 흐름을 재현한 상태는 아닙니다. HTTPS 전송, PKCE 검증, Chrome 리디렉션 정책은 각각 확인할 조건입니다.
      </Typography>

      <Typography component="li" sx={{mb:0.5}}>
        Spring Cloud 모듈 종류 (Kiali - 트래픽 모니터링)
      </Typography>
      <ZoomableImageModal imageSrc="/images/kiali.png" altText="Spring Cloud 모듈 종류 (Kiali - 트래픽 모니터링)" sx={{mb:-1}} />
      
      <Box sx={{mt:2}}>
        <CodeAccordion title="당시 background.js 인증 함수 목록 — 공개 체크아웃과 구분" language="javascript" codeString={backgroundCode} />

        <CodeAccordion title="당시 백엔드 기능 발췌 — 현재 공개 소스 범위는 개선 검토 참고" language="java" codeString={springCode} />
      </Box>
    </Box>
  );
}
